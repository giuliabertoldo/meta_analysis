...
```{r}
levels(df2$development_stage)
# Subset to include only studies with homogeneous groups
# meaning: exclude "Wide range" and "Wide Range"
df2_develop <- filter(df2, 
                      (development_stage == "Adolescents") | 
                        (development_stage == "Adults") |
                        (development_stage == "Children"))

df2_develop$development_stage <- droplevels(df2_develop$development_stage)
levels(df2_develop$development_stage)

# Fit meta-analysis: DIFFERENT TAU^2
meta1_develop1 <- metacor(cor = r, 
                         n = n, 
                         studlab = reference, 
                         data= df2_develop, 
                         fixed = FALSE, 
                         ...
                         title = "Mindset and Academic Achievement", 
                         prediction = TRUE, 
                         subgroup = development_stage, 
                         tau.common = FALSE)
meta1_develop1

# Fit meta-analysis: EQUAL TAU^2
meta1_develop2 <- metacor(cor = r, 
                         n = n, 
                         studlab = reference, 
                         data= df2_develop, 
                         fixed = FALSE, 
...
## TO BE DONE: HOW TO COMPARE THE THREE CATEGOREIS (ANOVA WITH CORRECTION FOR MULTIPLE COMPARISON) 
```
...
# Include only levels of interest
...
# Drop level "Wide Range" 
df3 %>% filter(development_stage == "Wide Range")
df3_develop$development_stage <- droplevels(df3_develop$development_stage)
levels(df2_develop$development_stage)

meta1_multi_develop <- rma.mv(yi = yi, 
                              V = vi, 
                              random = list(~1 | study_id/es_id), 
                              data = df3_develop, 
                              mods = ~factor(development_stage)-1)
summary(meta1_multi_develop)
